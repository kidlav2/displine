import { CheckCircle2, Clock, AlertTriangle, XCircle, CalendarDays } from "lucide-react";
...
export function StatusBadge({ status }: StatusBadgeProps) {
  const cfg: Record<string, { label: string; icon: React.ReactNode; cls: string }> = {
    ...
    in_progress: { label: "В процессе", icon: <Clock size={11} />,          cls: "bg-amber-50 text-amber-600 border-amber-200"   },
    late:        { label: "Опоздание",  icon: <AlertTriangle size={11} />,  cls: "bg-amber-50 text-amber-600 border-amber-200"   },
    missing:     { label: "Отсутствует",icon: <XCircle size={11} />,        cls: "bg-muted text-muted-foreground border-border"  },
    ...
    postponed:   { label: "Перенос",    icon: <CalendarDays size={11} />,   cls: "bg-purple-50 text-purple-600 border-purple-200"},
  };
