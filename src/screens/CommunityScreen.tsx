...
export function CommunityScreen() {
  const { challenge, updateChallenge, isAdmin, adminTz } = useAppContext();
  ...
  const sorted = [...challenge.participants].sort((a, b) =>
    sort === "score" ? calcScore(b.results) - calcScore(a.results) : b.km - a.km
  );

  const onViewParticipant = (id: number) => navigate(`/participants/${id}`);

  const handleLike = (id: number) => updateChallenge(challenge.id,
    { feed: challenge.feed.map((f: FeedItem) => f.id === id ? { ...f, liked: !f.liked, likes: f.liked ? f.likes - 1 : f.likes + 1 } : f) }
  );

  const handleComment = (id: number, text: string) => updateChallenge(challenge.id,
    ...
    <div className="px-4 lg:px-6 pt-5 lg:pt-8 max-w-[720px] mx-auto">
      ...
      {tab === "achievements" && (
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 pb-4">
          {ACHIEVEMENTS.map(a => (
            <Card key={a.title} className={`!p-4 ${!a.unlocked ? "opacity-40" : ""}`}>
