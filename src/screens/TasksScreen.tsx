...
import { Camera, ImageIcon, CheckCircle2, XCircle, Clock, ExternalLink, Lock } from "lucide-react";
...
export function TasksScreen() {
  ...
  const canSubmit = photo && (type === "task" || dist.length > 0);
  const submit = () => {
    if (!canSubmit) return;
    setStatus("pending");
  ...
  return (
    <div className="max-w-[560px] mx-auto px-4 lg:px-6 pt-5 lg:pt-8 space-y-4 pb-6">
      ...
      <button
        ...
        className={`w-full h-48 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-3 transition-colors ${photo ? "border-green-400 bg-green-50" : "border-border bg-card"}`}
      >
        {photo
          ? <><CheckCircle2 size={32} className="text-green-500" /><p className="text-sm font-bold text-green-600">Photo added</p></>
          : <><div className="flex gap-5"><Camera size={28} className="text-muted-foreground" /><ImageIcon size={28} className="text-muted-foreground" /></div>
              <p className="text-sm font-bold">Camera / Gallery</p><p className="text-xs text-muted-foreground">Tap to add proof photo</p></>
        }
      </button>
      ...
      {type === "run" && (
        <>
          <Card className="!p-4">
            ...
            <div className="flex items-end gap-2 mt-2">
              <input type="number" placeholder="5.0" value={dist} onChange={e => setDist(e.target.value)}
                ...
                style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 44, fontWeight: 900, lineHeight: 1 }} />
              <span className="text-xl font-bold text-muted-foreground mb-1">km</span>
            </div>
          </Card>
          <button className="w-full py-3 rounded-xl border-2 border-border bg-card flex items-center justify-center gap-2 font-semibold text-sm">
            <ExternalLink size={14} /> Connect Strava / Apple Health
          </button>
        </>
      )}
